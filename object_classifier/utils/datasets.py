...
from object_classifier.utils.utils import xyxy2xywh, xywh2xyxy
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self,
        ...
        if self.rect:
            # Read image shapes
            sp = 'data' + os.sep + path.replace('.txt', '.shapes').split(
        ...
        if cache_images and augment:  # if training
            for i in tqdm(range(min(len(self.img_files), 10000)),
                          desc='Reading images'):  # max 10k images
...
def convert_images2bmp():
    ...
    for path in ['../coco/images/val2014/', '../coco/images/train2014/']:
        ...
        for f in tqdm(glob.glob('%s*.jpg' % path)):
            save_name = f.replace('.jpg',
                                  '.bmp').replace(folder, folder + 'bmp')
            cv2.imwrite(save_name, cv2.imread(f))

    for label_path in ['../coco/trainvalno5k.txt', '../coco/5k.txt']:
        with open(label_path, 'r') as file:
            lines = file.read()
        lines = lines.replace('2014/',
                              '2014bmp/').replace('.jpg', '.bmp').replace(
